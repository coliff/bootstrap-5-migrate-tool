...
function migrate() {
  return (
    src(['src/*.html'])
      ...
      .pipe(
        replace(
          /<script src="https:\/\/cdn\.cloudflare\.com\/ajax\/libs\/bootstrap\/4\.\d+\.\d+\/dist\/js\/bootstrap\.bundle(\.min)?\.js">/g,
      ...
      .pipe(replace(/\sdata-display=/g, ' data-bs-display='))
      ...
      .pipe(replace(/\sdata-popper-config=/g, ' data-bs-popper-config='))
      .pipe(replace(/\sdata-reference=/g, ' data-bs-reference='))
